...
module.exports.loginWithoutPasswordDefine = async (event, context) => {
  ...
  async function getAllUsersPaginated({
    ...
    const emsMapArrayPromise = dynamodb.scan({
      TableName: process.env.EMSMap,
    ...
    while (pgToken || pgToken === 0) {
      ...
      const result = await cognitoidentityserviceprovider.listUsers({
        UserPoolId: process.env.UserPoolId,
        Filter: user.isAdmin ? '' : (user.company !== user.username ? `username = "${user.username}"`:`family_name = "${user.username}"`),
        ...(pgToken ? {PaginationToken: pgToken} : {})
      }).promise();
    ...
    const emsMapArray = await emsMapArrayPromise;
    console.log(emsMapArray.length);
  
    const emsMap = emsMapArray.reduce((map, el) => {
      map[el.userSub] = {
        target: el.target,
      };
      return map;
    ...
    const formatedUsers = users.map( (user, index) => {
      ...
      const phone_number = user.Attributes.find((attr)=>attr.Name == "phone_number")
      const phone_number_verified = user.Attributes.find((attr)=>attr.Name == "phone_number_verified")
     
      const group = user.Attributes.find((attr)=>attr.Name == "custom:group")
      ...
      return {
        ...
        phone_number_verified: (phone_number_verified && phone_number_verified.Value) || "",
        emailVerified: (emailVerified && emailVerified.Value) || "",
